...
add_filter( 'upload_dir', 'ic_woo_ic_page_upload_dir_enable', 9 );

function ic_woo_ic_page_upload_dir_enable( $dir ) {
    $id = ( isset( $_REQUEST['post_id'] ) ? $_REQUEST['post_id'] : '' );
    if ( $id && get_post_type( $id ) === 'product' ) {
        $enabled   = ic_is_page_for_woo_enabled();
        $ic_woocat = ic_woocat_settings();
        if ( ! empty( $enabled ) || empty( $ic_woocat['catalog']['enable'] ) ) {
            add_filter( 'product_post_type_array', 'ic_woo_ic_post_type_page_force_enable' );
        }
    }
...
function ic_woo_ic_post_type_page_force_enable( $array ) {
    $array[] = 'product';
