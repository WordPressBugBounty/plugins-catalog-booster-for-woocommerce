...
function ic_woocat_settings_page() {
    ...
    <div id="implecode_settings" class="wrap">
        <h2><?php _e( 'Settings', 'catalog-booster-for-woocommerce' ) ?>
            - <?php _e( 'Catalog Booster for WooCommerce', 'catalog-booster-for-woocommerce' ) ?></h2>
                ...
                <form method="post" action="options.php">
                    ...
                    if ( empty( $tab ) || $tab == 'general' ) {
                        ...
                        if ( class_exists( 'ic_shopping_ecommerce_cart' ) ) {
                            implecode_settings_radio( __( 'Cart Mode', 'catalog-booster-for-woocommerce' ), 'ic_woocat[general][cart_mode]', $ic_woocat['general']['cart_mode'], array(
                                    'woo' => 'WooCommerce',
                                    'epc' => 'eCommerce Product Catalog'
                            ) );
                        }
                        echo '</table>';
                    }
                    ...
                    <p class="submit">
                        <input type="submit" class="button-primary"
                               value="<?php _e( 'Save changes', 'catalog-booster-for-woocommerce' ); ?>"/>
                    </p>
                </form>
            </div>
        </div>
        ...
        <div class="plugin-logo">
            <a href="https://implecode.com/#cam=woocat-catalog-settings-link&key=logo-link"><img class="en"
                                                                                                 src="<?php echo IC_WOOCAT_BASE_URL . 'img/implecode.png'; ?>"
                                                                                                 width="282px"
                                                                                                 alt="impleCode"/></a>
        </div>
    </div>
...
function ic_woocat_settings() {
    ...
    $ic_woocat['general']['cart_mode']       = isset( $ic_woocat['general']['cart_mode'] ) ? $ic_woocat['general']['cart_mode'] : 'epc';
...
function ic_woocat_general_menu() {
    ...
    ?>
    <a id="general-settings" class="nav-tab <?php echo $general ?>"
       href="<?php echo admin_url( 'admin.php?page=ic-catalog-mode&tab=general' ) ?>"><?php _e( 'General', 'catalog-booster-for-woocommerce' ); ?></a>
    <a id="general-settings" class="nav-tab <?php echo $button ?>"
       href="<?php echo admin_url( 'admin.php?page=ic-catalog-mode&tab=button' ) ?>"><?php _e( 'Button', 'catalog-booster-for-woocommerce' ); ?></a>
    <a id="general-settings" class="nav-tab <?php echo $listing ?>"
       href="<?php echo admin_url( 'admin.php?page=ic-catalog-mode&tab=listing&submenu=archive-design' ) ?>"><?php _e( 'Product Listing', 'catalog-booster-for-woocommerce' ); ?></a>
    <a id="general-settings" class="nav-tab <?php echo $page ?>"
       href="<?php echo admin_url( 'admin.php?page=ic-catalog-mode&tab=product_page' ) ?>"><?php _e( 'Product Page', 'catalog-booster-for-woocommerce' ); ?></a>
    <a id="general-settings" class="nav-tab <?php echo $catalog ?>"
       href="<?php echo admin_url( 'admin.php?page=ic-catalog-mode&tab=catalog' ) ?>"><?php _e( 'Additional Catalog', 'catalog-booster-for-woocommerce' ); ?></a>
    <?php if ( function_exists( 'start_implecode_install' ) ) { ?>
        <a id="woocatextensions-settings" class="nav-tab <?php echo $extensions ?>"
           href="<?php echo admin_url( 'admin.php?page=ic-catalog-mode&tab=woocat_extensions' ) ?>"><?php _e( 'Addons & Integrations', 'catalog-booster-for-woocommerce' ); ?></a>
        <?php
    }
    ?>
    <a id="woocathelp-settings" class="nav-tab <?php echo $help ?>"
       href="<?php echo admin_url( 'admin.php?page=ic-catalog-mode&tab=woocat_help' ) ?>"><?php _e( 'Help', 'catalog-booster-for-woocommerce' ); ?></a>
    <?php
}
...
function ic_woocat_help_settings( $tab, $ic_woocat ) {
    if ( $tab == 'woocat_help' ) {
        ...
        <style>#implecode_settings .settings-wrapper p.submit {
                display: none
            }</style>
        <div id="implecode_settings" class="wrap">
            <?php
            ...
            <div class="help">
                ...
                <p>If you also install free <a
                            href="<?php echo admin_url( 'plugin-install.php?s=implecode+eCommerce+Product+Catalog+Plugin+for+WordPress+powerful+and+free&tab=search&type=term' ) ?>">eCommerce
                        Product Catalog</a> you will be able to:</p>
                <ol>
                    ...
                    <li>Many more countless possibilities with <a target="_blank"
                                                                  href="https://implecode.com/wordpress/plugins/#cam=woocat-extensions-help&key=extensions">impleCode
                            extensions</a></li>
                </ol>
                ...
                <a class="button-secondary" target="_blank"
                   href="https://wordpress.org/support/plugin/catalog-booster-for-woocommerce/">Free Support Forum</a>
                <a class="button-secondary" target="_blank"
                   href="https://implecode.com/support/#cam=woocat-extensions-help&key=support">Premium Support
                    Service</a>
                <?php if ( function_exists( 'start_implecode_install' ) ) { ?>
                    <h3><?php _e( 'How to Install the extension?', 'ecommerce-product-catalog' ) ?></h3>
                    <ol>
                        <li>Go to the <a
                                    href="<?php echo admin_url( 'admin.php?page=ic-catalog-mode&tab=woocat_extensions' ) ?>">extensions
                                page</a></li>
                    ...
                    <p>In case you prefer to install the extension manually you will get also the installation files in
                        the customer panel. See <a
                                href="https://implecode.com/wordpress/product-catalog/plugin-installation-guide/#cam=woocat-extensions-help&key=manual-installation#manual">manual
                            installation guide</a> for this.</p>
                    <p>Please see the <a href="https://implecode.com/faq/#cam=woocat-extensions-help&key=faq">FAQ</a>
